...
export class ExamRecordsModel {
    ...
    static async getIncomeOrEgressExamRecords(userDocument, addToQuery = ''){
        const result = await conn.request()
         .input('userDocument', sql.NVarChar, userDocument)
         .query(`
            SELECT 
                chit.checklist_id, chit.checklist_item_id, tpem.tipo_examen_id, chit.estado AS estado_item, 
                chem.estado AS esta_activo, tpem.nombre AS tipo_examen, tpem.estado AS estado_tipo_examen,
                chit.observaciones, chit.fecha_realizado, chit.fecha_vencimiento, chit.frecuencia_dias, 
                chit.PDF_url, chem.cc_empleado, chem.es_requerido
            ...
            WHERE chit.estado NOT IN ('Eliminado') AND chem.cc_empleado = @userDocument ${addToQuery}
        `);
